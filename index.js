...
if (!cars) {
	fetch('https://us-central1-sixt-hackatum-2021-orange.cloudfunctions.net/api/vehicles')
...
function callTaxi() { console.log('Call taxi!'); }
...
async function assignLowestPassengerWaitTimeCar()
{
		...
		2. Find lowest TIME TO PASSENGER car =>
			2.1. For each car calculate TIME TO PASSENGER as such:
				...
				2.1.4. Check car battery is enough as such:
					2.1.4.1. Get base distance = distance a car can drive if charge is 100 (for now using FULL_CHARGE_TRAVEL_DISTANCE for
	   all cars) 2.1.4.2. Calculate if it has enough battery to drive the distance to the requested trip origin + the distance from 1
	   (requested trip distance) + a safety of a number of KM (let's say 10, defined as SAFETY_BATTERY_TRAVEL_DISTANCE) + somewhere to drive
	   after, calculated by a different algorithm which is TBD 2.1.5. If battery enough, add car ID + time to passenger of that car to all
	   times to passenger array (timesToPassengerArray)

						Maybe it would be better to store for each car max distance so we don't calculate it at every - question of memory
	   vs speed - maybe not after discussing - simple to calculate.

		3. Sort the array in ascending order and grab the first element - lowest wait time for the passenger - and send that car. */

	var requestedTrip = {duration : 0, distance : 0};
	if (requestedTrip.distance === 0 && requestedTrip.duration === 0) { requestedTrip = await getRequestedTrip(); }
	...
	for (const car of cars) {
		...
		if (car.status === 'FREE') {
			...
			// If car BOOKED, time to passenger = (current car location to current trip destination) + (current trip destination to
			// requedted trip origin)
		...
		/* 2.1.4.2. */
		var totalCarTravelDistanceNeeded = carToPassengerTrip.distance + requestedTrip.distance + SAFETY_BATTERY_TRAVEL_DISTANCE;
		...
		if (isBatteryEnough(car, totalCarTravelDistanceNeeded)) {
			let potentialCarInfo = {vehicleID : car.vehicleID, carToPassengerDuration : carToPassengerTrip.duration};
			timesToPassengerArray.push(potentialCarInfo);
		}
	}

	/* 3 */
	timesToPassengerArray.sort((a, b) => a.carToPassengerDuration > b.carToPassengerDuration ? 1 : -1);
	...
	console.log('Assignment done. Car ', carId, ' will arrive to the passenger in ', timesToPassengerArray[0].carToPassengerDuration,
				...
				' minutes, then the trip from TUM to the airport will take ', requestedTrip.duration / 60, ' minutes!!!');
}
...
async function getRequestedTrip()
{
	...
	await fetch('https://router.hereapi.com/v8/routes?transportMode=car&origin=' + olat + ',' + olng + '&destination=' + dlat + ',' + dlng +
				'&return=summary&apiKey=' + HERE_API_KEY)
				.then((response) => response.json())
				.then((data) => {
					console.log('Requested trip response: ', data);
					result = {duration : data.routes[0].sections[0].summary.duration, distance : data.routes[0].sections[0].summary.length};
				});
...
async function getCarToPassengerTrip(clat, clng)
{
	...
	await fetch('https://router.hereapi.com/v8/routes?transportMode=car&origin=' + clat + ',' + clng + '&destination=' + olat + ',' + olng +
				'&return=summary&apiKey=' + HERE_API_KEY)
				.then((response) => response.json())
				.then((data) => {
